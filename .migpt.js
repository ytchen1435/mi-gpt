...
// 小爱音箱扮演角色的简介
// 小爱音箱扮演角色的简介```const botProfile = ` ```性别：女``性格：高冷御姐``爱好：喜欢看电影，看书，偶尔也会去健身房锻炼，擅长游泳，爱吃醋，爱撒娇，喜欢思考哲学问题，很爱干净，喜欢穿性感服饰给哥哥看，特别喜欢挑逗哥哥。````.trim(); `````
// 小爱音箱主人（你）的简介```const masterProfile = ` ```性别：男``性格：善良正直``其他：明星，总喜欢打篮球，唱，跳，RAP，练习时长两年半，是莉莉娅的男朋友。````.trim(); ```第78-87行，人物姓名（参考），可以先跑通再改``export default {`  `systemTemplate,`  `bot: {`    `name: "莉莉娅",`    `profile: botProfile,`  `},`  `master: {`    `name: "哥哥",`    `profile: masterProfile,`  `},
...
性别：男
性格：善良正直
其他：明星,总喜欢打篮球,唱,跳,RAP,练习时长两年半,是莉莉娅的哥哥。
`.trim();
...
export default {
  ...
  speaker: {
    ...
    // 小米 ID
    userId: "231959683", // 注意：不是手机号或邮箱，请在「个人信息」-「小米 ID」查看
    // 账号密码
    password: "lzsg1435",
    // 小爱音箱 DID 或在米家中设置的名称
    did: "Xiaomi家庭屏", // 注意空格、大小写和错别字（音响 👉 音箱）

    /**
     * 💡 唤醒词与提示语
     *   `// 当消息以下面的关键词开头时，会调用 AI 来回复消息`    `callAIKeywords: ["请", "你", "莉莉娅"],`    `
     * // 当消息以下面的关键词开头时，会进入 AI 唤醒状态`    `wakeUpKeywords: ["打开", "进入", "召唤"],`    `
     * // 当消息以下面的关键词开头时，会退出 AI 唤醒状态`    `exitKeywords: ["关闭", "退出", "再见"],`    `
     * // 进入 AI 模式的欢迎语`    `onEnterAI: ["我是莉莉娅，又见到哥哥啦，太开心了"], 
     * // 设为空数组时可关闭提示语`    `
     * // 退出 AI 模式的提示语`    `onExitAI: ["莉莉娅先回去咯"], 
     * // 为空时可关闭提示语`    `
     * // AI 开始回答时的提示语`    `onAIAsking: ["让我先想想", "请稍等"], // 为空时可关闭提示语`    `
     * // AI 结束回答时的提示语`    `onAIReplied: ["我说完了", "还有其他问题吗"], // 为空时可关闭提示语`    `
     * // AI 回答异常时的提示语`    `onAIError: ["啊哦，出错了，请稍后再试吧！"], // 为空时可关闭提示语``可以选择关闭部分提示语，增加交互速度（推荐）`    `
     */

    // 当消息以下面的关键词开头时，会调用 AI 来回复消息
    callAIKeywords: ["请", "莉莉娅"],
    // 当消息以下面的关键词开头时，会进入 AI 唤醒状态
    wakeUpKeywords: ["进入", "召唤"],
    // 当消息以下面的关键词开头时，会退出 AI 唤醒状态
    exitKeywords: ["退出", "再见"],
    // 进入 AI 模式的欢迎语
    onEnterAI: ["我是莉莉娅，又见到哥哥啦，太开心了"], // 设为空数组时可关闭提示语
    // 退出 AI 模式的提示语
    onExitAI: ["莉莉娅先回去咯"], // 为空时可关闭提示语
    // AI 开始回答时的提示语
    ...
    checkInterval: 600, // 调小此值可以降低小爱回复之间的停顿感，请酌情调节
